# cpp_timer
Есть 2 режима:
1) Режим "Таймер", в котором:
+ кнопка управления запускает таймер
+ кнопка режима на нажатие останавливает таймер и сбрасывает, если уже на паузе. А при зажатии переключает режим
3) Режим "Настройки", в котором:
  + кнопка управления добавляет и убавляет секунды
  + кнопка режима на нажатие переключает добавление\убавление секунд. А зажатие переключает режим
